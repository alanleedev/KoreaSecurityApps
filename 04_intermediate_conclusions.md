...
그러나 키보드 입력 내용을 암호화해서 웹사이트에 전달하면, 악성코드가 이걸 보지 못할 것이라는 생각인 것이다.
...
인터넷 뱅킹을 노린 트로이목마가 등장하여 패스워드를 훔치는 것은 이미 20여년 전부터 일어나고 있었다.
...
> OTP는 계속하여 바뀌는 일회성 패스워드이므로, 해커가 OTP 번호를 완전 실시간으로 확보하여 쓰지 못하는 이상 안전하다는 것을 보장할 수 있는 체제입니다.
